...
Multi-market topline view of the moth-control category. Same layout as Atopic Skin (DE / FR / IT / ES) — single-page topline with revenue/units per market, segment × market heatmap, top brands per segment.
...
├── data/
│   ├── x-ray/{DE,FR,IT,ES}/         ← drop Moth-{CODE}.csv here
│   ├── sales-data/{DE,FR,IT,ES}/    ← per-ASIN daily sales (optional)
│   └── competitor-listings/{DE,FR,IT,ES}/
...
1. **Drop X-Ray CSVs** into `data/x-ray/{CODE}/Moth-{CODE}.csv` (one per market: DE/FR/IT/ES)
...
Registered in `projects/Console/config.json` under the `topline` group as `Moth Control (DE, FR, IT, ES)`.
