...
{table_rows_html}      </tbody>
  ...
  <!-- Methodology -->
  <div class="note">
    <strong>Metodologia:</strong> Helium 10 X-Ray - snapshot 30-dniowy per rynek (2026-07-29), przeskalowany na 12 miesięcy z korektą sezonowości. Mnożnik = 12 / indeks_miesiąca, gdzie krzywa sezonowa pochodzi z dashboardu Fruit Fly Trap DE i jest liczona <strong>wyłącznie z 4 ASIN-ów z pełnym pokryciem 12 miesięcy</strong> (Aeroxon, Novokill &times;2, PIC; średni miesiąc = 1.0). ASIN-y z niepełną historią (m.in. Super Ninja) są celowo wykluczone - ich fragmentaryczne dane zniekształcałyby krzywą (sztuczne szczyty w miesiącach z danymi). Eksport przypada na lipiec (indeks {SEAS_INDEX[EXPORT_MONTH]:.2f}, wysoki sezon much owocowych), więc mnożnik wynosi <strong>&times;{MULTIPLIER:.2f}</strong> - mniej niż płaskie &times;12, co koryguje zawyżenie wynikające z sezonowego szczytu. Ten sam mnożnik zastosowano do wszystkich {len(MARKETS)} rynków (wszystkie eksporty z tego samego okna). Duplikaty usunięte po ASIN w obrębie każdego rynku; wiersze bez ASIN lub bez ceny są wykluczone. Sztuki 30-dniowe oszacowane jako ASIN Revenue podzielony przez cenę katalogową. Przychody i ceny z UK przeliczono z GBP na EUR po kursie {GBP_EUR} na etapie konsolidacji danych; wszystkie wartości podane są w EUR.
    <br><br>
