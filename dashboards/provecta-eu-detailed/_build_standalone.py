# -*- coding: utf-8 -*-
"""Build standalone index.html for the Provecta EU (Detailed) dashboard.

4-tab standalone, ALL static UI text in ENGLISH, NO segment dimension:
  Tab 1 · Total Market        - cross-market topline (generated by _build_rynek.py)
  Tab 2 · Market Structure    - per country, renderMarketStructure(bucket)
  Tab 3 · Reviews VOC         - per country, renderReviews(bucket)
  Tab 4 · Marketing Deep Dive - per country, renderMarketingDeepDive(bucket)

Markets: FR / DE / UK / IT / ES (order by market size).

Data sources (all read at build time, inlined into index.html):
  * Tab 1  : _rynek_topline.html (regenerated by _build_rynek.py on every build;
             its <style> and <body> are pasted VERBATIM into the Total Market panel)
  * Struct : data/x-ray/Provecta-{CODE}.csv  -> D.structure[CODE]
  * VOC    : reviews/{CODE}/voc.json         -> D.voc[CODE]  (or null)
  * MDD    : data/competitor-listings/{CODE}/mdd.json -> D.mdd[CODE] (or null)
...
MDD_TPL  = os.path.join(BASE, 'templates', 'tabs', 'u-marketing-deep-dive', 'template.html')
MS_TPL   = os.path.join(BASE, 'templates', 'tabs', 'market-structure', 'template.html')
ENGINE_JS_PATH = os.path.join(CONSOLE, 'js', 'data-engine.js')

# ── Identity ────────────────────────────────────────────────────────────────
PRODUCT_TITLE = 'Provecta EU - Detailed (FR, DE, UK, IT, ES)'
DASHBOARD_H2  = 'Provecta EU - Analiza szczegółowa'
DASHBOARD_SUB = 'Rynki: Francja · Niemcy · Wielka Brytania · Włochy · Hiszpania · Dane: Helium 10 X-Ray + analiza recenzji (VOC) + Marketing Deep-Dive'

# Per-market X-Ray Google Sheet links - placeholder '#' until sheets exist.
# Buttons are rendered ONLY for entries that are not '#'.
XRAY_LINKS = {'FR': '#', 'DE': '#', 'UK': '#', 'IT': '#', 'ES': '#'}

# ── Countries (order by market size FR -> DE -> UK -> IT -> ES) ─────────────
COUNTRIES = [
...
CURRENCY_BY_COUNTRY = {'FR': '€', 'DE': '€', 'UK': '£', 'IT': '€', 'ES': '€'}
TLD_BY_COUNTRY      = {'FR': 'fr', 'DE': 'de', 'UK': 'co.uk', 'IT': 'it', 'ES': 'es'}
...
voc_data = {c['code']: load_json(os.path.join(BASE, 'reviews', c['code'], 'voc.json'), f"VOC {c['code']}") for c in COUNTRIES}
mdd_data = {c['code']: load_json(os.path.join(BASE, 'data', 'competitor-listings', c['code'], 'mdd.json'), f"MDD {c['code']}") for c in COUNTRIES}

VOC_COUNTRY_CODES = [c['code'] for c in COUNTRIES if voc_data[c['code']]]
MDD_COUNTRY_CODES = [c['code'] for c in COUNTRIES if mdd_data[c['code']]]


# ── Market Structure: per-country products from X-Ray (whole market, 1 segment) ──
# 12M projection = 30d x 12 (no per-ASIN sales history). units30d = revenue/price.
def _pnum(val):
...
def load_structure(code):
    ...
    with open(path, encoding='utf-8-sig', newline='') as f:
        ...
        for r in rd:
            ...
            if price <= 0:
                continue
            if asin in best and rev30d <= best[asin]['revenue30d']:
                continue
            sales30d = round(rev30d / price) if rev30d > 0 else 0
...
# ── Tab 1: regenerate + extract topline style/body ──────────────────────────
def build_rynek():
    import subprocess
    r = subprocess.run([sys.executable, os.path.join(BASE, '_build_rynek.py')],
                       capture_output=True, text=True, encoding='utf-8')
    if r.returncode != 0:
        raise SystemExit('Total Market (Tab 1) build failed:\n' + (r.stderr or r.stdout))
    print('Total Market (Tab 1) rebuilt from local X-Ray -> _rynek_topline.html')


def extract_topline():
    with open(RYNEK_HTML, encoding='utf-8') as f:
        html = f.read()
    ...
    # Drop the topline's own <header> - the detailed shell has ONE header above the tabs.
...
# ── Dashboard-local functional patches on the VOC renderer (ENGLISH) ────────
# Scraped review ratings are critical-skewed / unreliable, so the headline
# rating + review count come from X-Ray. Rewrite the KPI row and hide the
# star-distribution bar. The "before" blocks are verbatim template code.
VOC_PATCH = [
  ...
  h += '</div></div></div>';""",
     "  // Star Distribution Bar - ukryty (rozkład ze scrape'u niereprezentatywny; średnia z X-Ray)"),
]

# ── Polish UI-string translations (static labels baked into the renderers) ──
# Data (themes, quotes, findings) is Polish in the JSON; these are the fixed
# English UI chrome strings hard-coded in the u-* renderers -> translate to Polish.
VOC_STR = [
...
MDD_STR = [
    ("No data loaded for this tab.", "Brak danych Marketing Deep-Dive dla tego rynku."),
...
def apply_patches(js, pairs):
    for before, after in pairs:
        if before not in js:
            raise SystemExit('VOC patch anchor not found - template changed? Anchor starts: ' + before[:80])
        js = js.replace(before, after)
    return js


def apply_translations(js, pairs):
    # Tolerant: chrome strings that are absent (e.g. removed by a patch) are skipped.
    for en, pl in pairs:
        js = js.replace(en, pl)
    return js
...
bundle = {
    ...
    'mddCountries': MDD_COUNTRY_CODES,
    'voc': voc_data,
    'mdd': mdd_data,
...
var state = { tab: 'market', country: D.defaultCountry };

/*<<VOC_FN>>*/
/*<<MDD_FN>>*/
/*<<MS_FN>>*/

// Structure shows all markets; VOC/MDD only the markets with data.
function countryCodesForTab(){
  if (state.tab === 'voc') return D.vocCountries || [];
  if (state.tab === 'mdd') return D.mddCountries || [];
  return D.countries.map(function(c){ return c.code; });
}
...
function scopeBucket(bucket, extra){
  ...
  out.countryTld = out.countryTld || (D.tldByCountry && D.tldByCountry[state.country]) || c.code.toLowerCase();
  out.segmentName = '';
...
function renderVOC(){
  ...
  var bucket = D.voc[state.country];
  if (!bucket) {
    root.innerHTML = '<div class="ad-empty">Brak danych VOC dla tego rynku.</div>';
    return;
  }
  root.innerHTML = '';
...
function renderMDD(){
  ...
  var bucket = D.mdd[state.country];
  if (!bucket) {
    root.innerHTML = '<div class="ad-empty">Brak danych Marketing Deep-Dive dla tego rynku.</div>';
    return;
  }
  root.innerHTML = '';
...
function renderStructure(){
  ...
  // Drop stale structure charts so re-render never touches a detached canvas.
...
function setTab(tab){
  ...
  // If the selected market has no data on this tab, fall back to the first available.
  var codes = countryCodesForTab();
  if (codes.length && codes.indexOf(state.country) === -1) state.country = codes[0];
...
buildCountryPills();
// Tab 1 (Total Market) is active at load so the topline charts render correctly.
</script>
</body>
</html>
'''
...
XRAY_BUTTONS = ''.join(
    '<a class="ad-xray-btn" href="{href}" target="_blank" rel="noopener">{svg}{code} X-Ray</a>'.format(
        href=XRAY_LINKS[c['code']], svg=_XRAY_SVG, code=c['code'])
    for c in COUNTRIES if XRAY_LINKS.get(c['code'], '#') != '#')
...
# House style: no long dashes anywhere in the rendered dashboard.
...
print('Default market: {}'.format(DEFAULT_COUNTRY))
for c in COUNTRIES:
    code = c['code']
    print('  {}: VOC={} | MDD={} | Structure ASINs={}'.format(
        code, 'yes' if voc_data[code] else 'NO', 'yes' if mdd_data[code] else 'NO', len(structure_data[code])))
