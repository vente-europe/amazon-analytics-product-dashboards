...
4. `python _assemble_voc.py` → `voc-lure.json` / `voc-electric.json`. Two extras here: (a) the review-browser list is **deduped by full normalized text** before the 200-cap, so variant-syndicated reviews never show twice (esp. adjacent after the date sort); (b) each `negativeTopic` gets a **real `foundIn` string** — `NEG_TOPIC_REGEX` (per segment, keyed by AI topic label) counts matching negative (1-3★) pool reviews → `"Found in X of Y negative reviews (Z%)"`, replacing the AI-estimated `pct` with a verifiable regex count (standalone-style) and re-sorting topics by real prevalence. `positiveTopics` get the same treatment via `POS_TOPIC_REGEX` (denominator = 4-5★ positive reviews) → "Found in X of Y positive reviews (Z%)". (d) `customerExpectations` (Unmet Needs) are sorted by pct desc, and `reorder_insights_to_unmet` re-orders the `negativeInsights` table to follow that exact sequence via the `INSIGHT_UNMET` map (each insight ↔ its 1:1 unmet need), so the two tables read in the same order. Regexes are heuristics (efficacy topics broadened to the dominant "doesn't work / caught nothing" language) — approximate, like the standalone's `neg_themes`. (c) `strip_insight_pct` removes the leading quantity from each `negativeInsight` finding ("Over 40% of complaints…" → "Complaints…") — the AI estimates were stale and, for DIY, misleadingly high (the regex over-counted bare "vinegar" mentions). Per request the insights are now qualitative; the verifiable count lives only in the topic dropdown. The `Overpriced DIY Substitute` regex was also tightened to require an explicit comparison/value signal (not bare "apple cider"/"vinegar"), dropping it 22.9% → 19.7%. Same fix applied to Electric: `Homemade ACV Beats Electric Traps` (bare vinegar removed) 9.6% → 6.6%, and `Dressed-up Fly Paper Gimmick` (bare "sticky pad" removed — the product *is* a sticky-pad trap) 6.6% → 3.5%.
